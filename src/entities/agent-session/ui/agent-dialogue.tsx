...
import { MarkdownTypewriter } from "@/shared/ui";
...
export function AgentDialogue({
  ...
  return (
    <div
      className={cn(
        "flex flex-col h-[calc(100vh-300px)] min-h-[500px] border rounded-xl overflow-hidden bg-muted/30",
        className,
      )}
    >
        ...
        <div className="space-y-6 pb-4">
          ...
          <AnimatePresence initial={false}>
            {dialogue.map((item, index) => {
              const isLastItem = index === dialogue.length - 1;
                ...
                <div key={item.question.id} className="space-y-4">
                  ...
                  >
                    ...
                    <div className="flex-1 max-w-[90%]">
                      <div className="bg-card border rounded-2xl rounded-tl-sm p-4 shadow-sm hover:scale-[1.01] transition-transform duration-200">
                        <div className="flex items-start justify-between gap-2 mb-2">
                          ...
                          <QuestionStatusBadge
                            status={
                              item.answer ? "answered" : item.question.status
                            }
                          />
                        </div>
                        <div className="prose prose-sm dark:prose-invert max-w-none text-sm break-words">
                          {isLastItem && !item.answer ? (
                            <MarkdownTypewriter
  ...
  return (
    <motion.span
      ...
      className={cn(
        ...
        !["answered", "skipped", "unanswered"].includes(status) &&
